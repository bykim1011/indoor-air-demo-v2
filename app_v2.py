...
with st.expander("법정 악취 배출허용기준 참고", expanded=False):
    st.markdown(
        """
        아래 기준은 「악취방지법 시행규칙」 별표 3의 배출허용기준 중
        본 데모와 관련된 주요 항목을 정리한 참고용 기준입니다.
        
        현재 화면의 악취측정값은 서구 자체 측정망의 센서법 기반 간이 측정치이므로,
        법정 공인측정 결과나 행정처분 판단 기준으로 직접 해석해서는 안 됩니다.
        """
    )

    odor_standard_df = pd.DataFrame([
        {
            "구분": "암모니아",
            "표기": "NH₃",
            "단위": "ppm",
        ...
        },
        {
            ...
            "표기": "희석배수",
            "단위": "배",
            "공업지역 배출허용기준": "부지경계선 20 이하",
            "기타지역 배출허용기준": "부지경계선 15 이하",
            "비고": "두 가지 이상 악취물질의 복합 냄새"
        }
    ])
    ...
    st.caption(
        "자료: 악취방지법 시행규칙 별표 3. "
        "TVOC는 현 측정망 표시 항목이지만, 위 별표 3의 지정악취물질 배출허용기준 표에서 "
        "TVOC라는 통합 항목으로 직접 제시되지는 않습니다."
    )
